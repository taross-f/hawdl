...
> 二重管理を避けるため一箇所に集約している。tap 側が毎時こちらを見に来て最新リリースに
> 自分を更新するので、タグを打てば1時間以内に `brew` に届く。手で formula を書き換える必要はない。
...
tap の同期自体も、新しい tarball のチェックサムを取るためにリリースごとに1回だけ計上される。
大まかな規模感として読むこと。
...
システムの第一言語が日本語なら日本語、それ以外なら英語で表示する。
システム設定 → 一般 → 言語と地域 の優先言語リストの**先頭**だけを見る。
ドイツ語・日本語の順に並んでいる環境では英語になる。このアプリにドイツ語は無く、
用意してある 2 言語のうちでは英語のほうが近いため。

言語は起動時に一度だけ読む。システムの言語を変えた場合は再起動が必要で、
これは正しくローカライズされたアプリでも同じ。

システム設定によらず言語を固定したい場合:
...
戻すときは `defaults delete com.github.taross-f.hawdl.HawdlBar AppleLanguages`。

文字列は `.lproj` リソースではなくバイナリに埋め込んでいるため、
**システム設定 → 一般 → 言語と地域 → アプリケーション には出てこない**。
あそこに並ぶのはローカライズを宣言しているアプリだけ。
判断の理由は `Sources/HawdlCore` の `UILanguage` に書いてある —
`.app` は複数箇所で手作業で組み立てており、リソースディレクトリの
コピー漏れは無言の失敗になるため。
